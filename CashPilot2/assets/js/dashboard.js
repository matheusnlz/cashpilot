...
function inicializarGraficoEvolucao(dados, canvasId = 'graficoEvolucao') {
    ...
    if (!Array.isArray(dados) || dados.length === 0) {
        canvas.parentElement.innerHTML = '<p class="texto-vazio">Ainda não há movimentações suficientes para exibir o gráfico.</p>';
        return;
    }
        ...
        options: {
            ...
            plugins: {
                ...
                tooltip: {
                    callbacks: {
                        label(context) {
                            return `${context.dataset.label}: ${Number(context.raw || 0).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}`;
                        },
                    },
                },
            },
