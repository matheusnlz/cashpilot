...
def gerar_insights(usuario_id: int) -> list:
    ...
    # --- Insight: capacidade de economia ---
    if receitas_atual > 0:
        taxa_economia = (saldo_atual / receitas_atual) * 100
        if taxa_economia < 0:
            insights.append({"tipo": "alerta", "mensagem": f"Suas despesas ultrapassaram suas receitas em {abs(taxa_economia):.0f}% neste mês."})
        else:
            insights.append({"tipo": "info", "mensagem": f"Você manteve {taxa_economia:.0f}% da sua renda disponível neste mês."})
    ...
    # --- Insight: principal origem de renda ---
    cursor.execute(
        """
        ...
        WHERE r.usuario_id = %s AND r.data_receita BETWEEN %s AND %s
        GROUP BY nome ORDER BY total DESC LIMIT 1
        """,
        (usuario_id, inicio_atual, fim_atual),
    )
    origem_renda = cursor.fetchone()
    if origem_renda and receitas_atual > 0:
        percentual = float(origem_renda["total"]) / receitas_atual * 100
        insights.append({"tipo": "info", "mensagem": f"{origem_renda['nome']} é sua principal origem de renda neste mês ({percentual:.0f}% das receitas)."})

    # --- Insight: gasto recorrente nos últimos 90 dias ---
    cursor.execute(
        """
        SELECT descricao, COUNT(*) AS ocorrencias, SUM(valor) AS total
        FROM despesas
        WHERE usuario_id = %s AND data_despesa >= DATE_SUB(CURDATE(), INTERVAL 90 DAY)
        GROUP BY descricao HAVING COUNT(*) >= 2
    ...
    if recorrente:
        insights.append({"tipo": "info", "mensagem": f"{recorrente['descricao']} aparece {recorrente['ocorrencias']} vezes nos últimos 90 dias e soma R$ {float(recorrente['total']):,.2f}.".replace(",", "#").replace(".", ",").replace("#", ".")})
    ...
    if meta and meta["valor_meta"] > 0:
        ...
        if saldo_atual > 0 and float(meta["valor_meta"]) > float(meta["valor_atual"]):
            meses = (float(meta["valor_meta"]) - float(meta["valor_atual"])) / saldo_atual
            insights.append({"tipo": "info", "mensagem": f"Se reservar o saldo atual mensal, sua meta \"{meta['titulo']}\" pode ser alcançada em cerca de {max(1, round(meses))} mês(es)."})
