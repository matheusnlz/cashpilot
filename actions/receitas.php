...
if ($acao === 'criar' || $acao === 'editar') {
        ...
        if ($acao === 'criar') {
                ...
                if ($itemVendaId > 0) {
                        ...
                        $iv=$pdo->prepare('SELECT id,nome,tipo,preco_venda,custo_unitario FROM produtos_servicos WHERE id=:id AND usuario_id=:uid AND ativo=1');

            $iv->execute(['id'=>$itemVendaId,'uid'=>$usuarioId]);

            $item=$iv->fetch();

                        if($item) {

                $qtd=$item['tipo']==='servico'?1:max(1,(int)($_POST['quantidade_venda']??1));

                $valor=(float)$item['preco_venda']*$qtd;

                $descricao=$item['nome'].($item['tipo']==='produto'?' x '.$qtd:'');

                $categoriaId=cpCategoriaId($pdo,$usuarioId,$item['tipo']==='servico'?'Serviços':'Vendas','receita');
                ...
                $receitaId=(int)$pdo->lastInsertId();
                                ...
                                $v=$pdo->prepare('INSERT INTO vendas (usuario_id,receita_id,data_venda,valor_bruto,custo_total) VALUES (:uid,:rid,:data,:valor,:custo)');

                $v->execute(['uid'=>$usuarioId,'rid'=>$receitaId,'data'=>$data,'valor'=>$valor,'custo'=>(float)$item['custo_unitario']*$qtd]);
                ...
                $vi->execute(['vid'=>$vendaId,'pid'=>$itemVendaId,'nome'=>$item['nome'],'tipo'=>$item['tipo'],'qtd'=>$qtd,'preco'=>$item['preco_venda'],'custo'=>$item['custo_unitario']]);

                                $pdo->prepare('UPDATE receitas SET venda_id=:vid WHERE id=:rid')->execute(['vid'=>$vendaId,'rid'=>$receitaId]);
...
$retorno = ($_POST['retorno'] ?? '') === 'vendas.php' ? 'vendas.php' : 'receitas.php';
