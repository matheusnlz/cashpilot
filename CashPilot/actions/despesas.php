...
if ($acao === 'excluir') {
    $id = (int) ($_POST['id'] ?? 0);
    $stmt = $pdo->prepare('DELETE FROM despesas WHERE id = :id AND usuario_id = :uid');
...
if ($acao === 'criar' || $acao === 'editar') {
    ...
    if ($acao === 'criar') {
        $stmt = $pdo->prepare(
            'INSERT INTO despesas (usuario_id, categoria_id, valor, descricao, data_despesa)
        ...
        $id = (int) ($_POST['id'] ?? 0);
        $stmt = $pdo->prepare(
            'UPDATE despesas SET categoria_id = :cat, valor = :valor, descricao = :descricao, data_despesa = :data
