...
<form autocomplete="off" action="../actions/importar_confirmar.php" method="POST">
    <?= csrfCampo() ?>
    <div class="cartao">
        ...
        <table>
            ...
            <?php foreach ($preview['linhas'] as $i => $linha): ?>
                <tr class="linha-importacao" data-linha="<?= $i ?>" style="<?= $linha['possivel_duplicata'] ? 'background:#FBF3E9;' : '' ?>">
                    <td>
                    ...
                    <td>
                        <input type="hidden" name="linhas[<?= $i ?>][categoria_sugerida_id]" value="<?= (int)$linha['categoria_id'] ?>">
...
<script>
    ...
    // Mantém todos os dados no mesmo formulário, exibindo apenas 20 linhas por vez.
    ...
    anterior.addEventListener('click', () => { if (pagina > 1) { pagina--; mostrarPagina(); } });
    proxima.addEventListener('click', () => { if (pagina < totalPaginas) { pagina++; mostrarPagina(); } });
    mostrarPagina();
</script>
