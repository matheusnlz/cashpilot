...
<div class="grade-dupla">
    <div>
        <div class="cartao" style="margin-bottom:20px;">
            <div class="radar-titulo"><div><h3>Categorias de receita</h3><p class="secao-ajuda">As categorias padrão seguem seu tipo de conta; as criadas por você podem ser removidas.</p></div></div>
            <?php foreach ($receitasCat as $c): ?>
                <div style="display:flex; justify-content:space-between; align-items:center; padding:8px 0; border-bottom:1px solid var(--cor-borda);">
                    <span><span class="badge" style="background:<?= $c['cor'] ?>22; color:<?= $c['cor'] ?>;"><?= limpar($c['nome']) ?></span> <small class="categoria-origem"><?= $c['padrao'] ? 'Padrão do perfil' : 'Personalizada' ?></small></span>
                    <?php if (!$c['padrao']): ?>
                        <form autocomplete="off" action="../actions/categorias.php" method="POST" onsubmit="return confirm('Excluir categoria?');">
                            <?= csrfCampo() ?>
        ...
        <div class="cartao">
            <div class="radar-titulo"><div><h3>Categorias de despesa</h3><p class="secao-ajuda">Use “Outros” apenas quando não houver uma classificação adequada.</p></div></div>
