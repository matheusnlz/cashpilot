...
function termoRegraImportacao(string $descricao): string {
    $t = mb_strtoupper($descricao,'UTF-8');
...
try {
    ...
    foreach ($linhasPostadas as $linha) {
        ...
        if ($tipo === 'receita') { $stmtReceita->execute($parametros); } else { $stmtDespesa->execute($parametros); }
        if ($categoriaId && $categoriaSugeridaId && $categoriaId !== $categoriaSugeridaId) {
            $termo = termoRegraImportacao($descricao);
            if (mb_strlen($termo) >= 3) {
                try { $rg=$pdo->prepare('INSERT INTO classificacao_regras (usuario_id,termo,tipo,categoria_id) VALUES (:uid,:termo,:tipo,:cat) ON DUPLICATE KEY UPDATE categoria_id=VALUES(categoria_id)');$rg->execute(['uid'=>$usuarioId,'termo'=>$termo,'tipo'=>$tipo,'cat'=>$categoriaId]); } catch(Throwable $e) {}
            }
        }
        $importadas++;
    }
